...
def parse_cabins(file_name, config):
  ...
  for row in range(first_cabin_row, sheet.max_row + 1):
    ...
    # otherwise the next row is from another cabin -> add the existing cabin to the list of cabins
      ...
      sheet[DIN1_col + str(row)].value, sheet[DIN2_col + str(row)].value, sheet[BRE_col + str(row)].value,
...
def createPDF(CABINS_XLSX, OUTPUT_FILE, config):
  ...
  for cabin in cabins:
    ...
    # random quote 
    elements.append(Paragraph('"'+config['quote'][random.randrange(0,len(config['quote']))]+'"', quote_style))
...
def main():
  ...
  for xlsx in files:
    ...
    createPDF(xlsx, f"{path}/{folder_name}/{folder_name}_{xlsx.split('/')[-1].split('.')[0]}.pdf", config)

    print(f"OUTPUT: \n /{folder_name}/{folder_name}_{xlsx.split('/')[-1].split('.')[0]}.pdf")
